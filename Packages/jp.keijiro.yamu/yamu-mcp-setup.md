...
Codex reads MCP server config from `~/.codex/config.toml`. Example:

```toml
[mcp_servers.yamu]
command = "node"
args = ["<PROJECT_ROOT>/Packages/jp.keijiro.yamu/Node/mcp-server.js", "--port", "17932"]
```

If you install via UPM, replace the path with
`<PROJECT_ROOT>/Library/PackageCache/jp.keijiro.yamu@(HASH)/Node/mcp-server.js`.
